...
class Interview extends React.Component {
    ...
    componentDidMount() {
        get(`https://recruiteasy.herokuapp.com/api/interview/${this.state.id}`, null).then((res) => {console.log(res.applicants);this.setState({question: res.questions, title: res.title, host: res.host, applicants: res.applicants})})
    ...
    save = async (e) => {
        ...
        if(this.state.tab !== this.state.question.length){
            ...
            const res = await post('https://recruiteasy.herokuapp.com/api/applicant/', null, {
            ...
            const data = await patch(`https://recruiteasy.herokuapp.com/api/interview/${this.state.id}`, {'applicants': this.state.applicants+1})
      ...
      return (
        <div className='bg-gray-100 h-screen flex bg-local'>
            ...
            <div className="modal fade bg-black bg-opacity-60 fixed hidden top-0 left-0 w-full h-full outline-none overflow-x-hidden overflow-y-auto" id="load">
                ...
                <svg role="status" className="mr-2 w-12 h-12 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
