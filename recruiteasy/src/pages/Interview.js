...
class Interview extends React.Component {
    ...
    componentDidMount() {
        get(`http://localhost:8000/api/interview/${this.state.id}`, null).then((res) => {this.setState({question: res.questions, title: res.title, host: res.host, applicants: res.applicants})})
    ...
    save = async (e) => {
        ...
        if(this.state.tab !== this.state.question.length){
            ...
            const data = await patch(`http://localhost:8000/api/interview/${this.state.id}`, {'applicants': this.state.applicants+1})
